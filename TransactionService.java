...
public class TransactionService {
    ...
    public void detectAndFlagSuspiciousTransaction(Transaction transaction) {
        ...
        if (!userTransactions.isEmpty()) {
            ...
            if (transaction.getTransactionAmount() > upperThreshold && !transaction.getFlagged()) {
                transaction.setFlagged(true);
    ...
    // @Scheduled(cron = " 23 * * * * *") // Runs every minute
    // @CacheEvict(value = { "transactionsCache", "flaggedTransactionsCache",
    //         "distinctTransactionsCache" }, allEntries = true)
    // public void automateFlaggingForAllTransactions() {
    //     // Log the current time (minute)
    //     logger.info("Automated flagging triggered at minute: " + java.time.LocalTime.now().getMinute());

    //     List<Transaction> transactions = getAllTransactions();
    //     transactions.forEach(this::automateFlagging);
    // }
